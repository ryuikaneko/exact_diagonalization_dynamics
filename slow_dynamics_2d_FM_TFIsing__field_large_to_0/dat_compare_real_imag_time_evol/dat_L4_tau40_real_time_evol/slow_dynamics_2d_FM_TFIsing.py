...
#    parser.add_argument('-tau',metavar='tau',dest='tau',type=np.float,default=32.0,help='set tau (max time, default 32.0)')
    parser.add_argument('-tau',metavar='tau',dest='tau',type=np.float,default=40.0,help='set tau (max time, default 40.0)')
...
#
    ...
    ising_steps = [ising_i+i*(ising_f-ising_i)/(Nsteps-1) for i in range(Nsteps)]
    ising_steps_for_U = [ising_i+(i+0.5)*(ising_f-ising_i)/(Nsteps-1) for i in range(Nsteps)]
#
    print("Lx=",Lx)
    print("Ly=",Ly)
    print("N=",N)
    ...
    print("ising_steps: J(t)=",ising_steps)
    print("ising_steps_for_U: J(t+dt/2)=",ising_steps_for_U)
...
## prepare initial state: GS at t=0
    ...
    elif np.abs(ising_i)<1e-10:
        Ham0 = copy.deepcopy(Op_x) ## GS is |+x>
    else:
        Ham0 = copy.deepcopy(ising_i*Ham_zz + field_i*Ham_x) ## GS of TFI
#
...
#        ising_m = ising_steps[i]
##
## U(t-->t+dt) = exp(-i*dt*H(t+dt/2)) (smaller error O((dt)^2))
## H(t+dt/2) = H_zz + h(t+dt/2) H_x
        field_m = field_steps_for_U[i]
        ising_m = ising_steps_for_U[i]
##
        Ham = copy.deepcopy(ising_m*Ham_zz + field_m*Ham_x)
        mitHam = (-1j)*dt*Ham ## real time evol.
#        mitHam = (-1)*dt*Ham ## imag. time evol.
#        print("step t J h",i,time_m,ising_m,field_m)
#
...
#        print("psi",i,psi)
#
## calculate log fidelity -1/2 * log(|<psi_instant(t)|psi_evolute(t)>|^2)
        if i < Nsteps-1:
            if np.abs(field_steps[i+1])<1e-10:
                Ham_m = copy.deepcopy(Op_z)
            elif np.abs(field_steps[i+1])<1e-10:
                Ham_m = copy.deepcopy(Op_x)
            else:
                Ham_m = copy.deepcopy(ising_steps[i+1]*Ham_zz + field_steps[i+1]*Ham_x)
            ene,vec = scipy.sparse.linalg.eigsh(Ham_m,which='SA',k=2)
            log_fidel.append( -0.5*np.log( (np.abs(np.dot(np.conjugate(vec[:,0]),psi))**2
                / np.dot(np.conjugate(vec[:,0]),vec[:,0]) / np.dot(np.conjugate(psi),psi)).real ) )
#    print(ret)
    ene0 = [(np.dot(np.conjugate(ret[i]),Ham0.dot(ret[i])) / np.dot(np.conjugate(ret[i]),ret[i])).real for i in range(Nsteps)]
    ene1 = [(np.dot(np.conjugate(ret[i]),(ising_steps[Nsteps-1]*Ham_zz+field_steps[Nsteps-1]*Ham_x).dot(ret[i])) / np.dot(np.conjugate(ret[i]),ret[i])).real for i in range(Nsteps)]
...
#    enex = [(np.dot(np.conjugate(ret[i]),Ham_x.dot(ret[i])) / np.dot(np.conjugate(ret[i]),ret[i])).real for i in range(Nsteps)]
    ene = [(np.dot(np.conjugate(ret[i]),(ising_steps[i]*Ham_zz+field_steps[i]*Ham_x).dot(ret[i])) / np.dot(np.conjugate(ret[i]),ret[i])).real for i in range(Nsteps)]
...
#
    fig12 = plt.figure()
    fig12.suptitle("log fidelity")
    plt.plot(time_steps,log_fidel)
    plt.xlabel("time (from 0 to tau)")
    fig12.savefig("fig_log_fidel.png")
#
    fig13 = plt.figure()
    fig13.suptitle("log fidelity")
    plt.plot(time_steps[:int(Nsteps*0.65)],log_fidel[:int(Nsteps*0.65)])
    plt.xlabel("time (from 0 to tau)")
#    plt.ylim([0,0.2])
#    plt.xlim([0,0.65*tau])
    fig13.savefig("fig_log_fidel_short_time.png")
#
#    plt.show()
    end = time.time()
